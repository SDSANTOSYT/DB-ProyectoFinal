...
def eliminar_tutor(id_tutor: int, force: Optional[bool] = Query(False, description="Si true borra dependencias y luego el tutor")):
    ...
    try:
        ...
        return {
            "id_tutor": id_tutor,
            "mensaje": "Tutor eliminado forzosamente; dependencias borradas/desvinculadas",
            "detalles": {
                "asistencias_borradas": cant_asistencias,
                "notas_borradas": cant_notas,
                "aulas_desvinculadas": cant_aulas_actualizadas
            }
        }

    except HTTPException:
        # dejar que FastAPI maneje la excepción (ya tiene detalle)
        if conn:
            conn.rollback()
        raise
        ...
        if conn:
            conn.rollback()
        logger.exception(f"Error de integridad al eliminar tutor {id_tutor}: {e}")
        raise HTTPException(status_code=409, detail="Error de integridad al eliminar tutor")

    except Exception as e:
        if conn:
            conn.rollback()
        logger.exception(f"Error inesperado al eliminar tutor {id_tutor}: {e}")
